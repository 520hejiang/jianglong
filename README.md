...
读取记忆(D1) → ①焦点提取 → 检索相关历史(tag匹配) → ②单章细纲
   ...
   → ④正文生成(2500-3000字, 衔接上一章结尾, 反AI腔, 开头去重)
   → ⑤按需润色(检出AI味/超限/重写才触发, 省钱) → 抽取状态增量
   → 【硬规则校验】不过则重写(最多2次)
   → ⑥落库 + 应用增量(角色/伏笔/剧情) + 推进进度
```

**反 AI 味与省钱**：正文 Prompt 注入 `ANTI_SLOP` 铁律（禁烂俗套话/翻译腔/小作文结构，情绪靠演不靠报，章首章尾去套路，主角毒舌吐槽+低频克制粗口），并喂入"最近 5 章开头"强制换切入方式根治雷同。润色由 `POLISH_MODE` 控制：`auto`(默认) 用零成本的代码级 AI 味检测器（套话/比喻密度/段落均匀度）判断是否需要润色，干净稿直接跳过，**每章省下一次 LLM 调用**；`always` 恒润，`off` 从不润。
